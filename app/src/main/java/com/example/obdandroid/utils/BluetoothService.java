...
import static com.example.obdandroid.config.Constant.BT_TOAST;
import static com.example.obdandroid.config.Constant.CONNECTED_DEVICE_NAME;
import static com.example.obdandroid.config.Constant.DEVICE_NAME;
import static com.example.obdandroid.config.Constant.REC_DATA;
import static com.example.obdandroid.config.Constant.TOAST;

/**
 * 作者：Jealous
 * 日期：2021/9/8 0008
 * 描述：蓝牙服务类，包括蓝牙连接监听线程、连接线程、已连接线程
 */
public class BluetoothService {
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        //发送已连接设备名回UI
        sendString2UI(CONNECTED_DEVICE_NAME, DEVICE_NAME,device.getName());
        setState(CONNECTED);
    }
    ...
    private void connectionFailed() {
        ...
        //向UI发送连接失败通知
        sendString2UI(BT_TOAST,TOAST,"连接失败");
    }
    /**
     * 发送字符串会UI
     * @param what 什么类型
    ...
    private void connectionBreak() {
        ...
        BluetoothService.this.acceptWait();
        // 向UI发送连接断开通知
        sendString2UI(BT_TOAST,TOAST,"连接断开");
    }
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    // 将接受数据发回UI处理
                    if(bytes!=-1&&allowRec){
                        mHandler.obtainMessage(REC_DATA,bytes,-1,buffer).sendToTarget();
                    }
                } catch (IOException e) {
