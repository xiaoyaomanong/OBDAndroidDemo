...
public class WXPayEntryActivity extends BaseActivity implements IWXAPIEventHandler {
    ...
    public void initView() {
        ...
        btnOK.setOnClickListener(v -> {
            sendBroadCast(rechargeStatus, amount, "1", mealId);
            finish();
        });
    }
    ...
    private void sendBroadCast(int status, String amount, String channel, String mealId) {
        Intent intent = new Intent("com.obd.pay");//创建发送广播的Action
        intent.putExtra("payResult", status);//支付结果
        intent.putExtra("paymentChannels", channel);//支付渠道
